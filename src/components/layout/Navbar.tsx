    ...
    <header className="print:hidden sticky top-0 z-20 flex min-h-[72px] items-center gap-2 border-b border-stone-200 bg-white px-3 sm:px-4 md:px-6">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2">
        <button
          ...
          aria-label={language === 'en' ? 'Change language to Tamil' : 'Change language to English'}
        >
          <Languages size={19} />
          <span className="hidden xl:inline">{language === 'en' ? 'தமிழ்' : 'English'}</span>
        </button>
        <button
          type="button"
          onClick={() => openHelp()}
        ...
        >
          <HelpCircle size={19} />
          <span className="hidden min-[390px]:inline">Help</span>
        </button>
      </div>
    </header>
  );
