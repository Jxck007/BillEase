...
export function CanonicalInvoiceDocument({ invoice, profile, customer, showQr = true, copyLabel = '' }: Props) {
  ...
  const amountInWords = `${numberToWordsIndian(Math.floor(invoice.total || 0))} rupees only`;
    ...
    <div className="invoice-print-page bg-white text-black" data-export-root="true">
      <div className="tv-container">
        ...
        <section className="tv-buyer">
          <div className="tv-sub">Buyer</div>
        ...
        <section className="tv-table-wrap overflow-x-auto">
          <table className="tv-table">
            ...
            <tfoot>
              <tr><td colSpan={5} className="tv-td-right">Subtotal</td><td className="tv-td-right">{formatCurrency(invoice.subtotal || 0)}</td></tr>
              ...
              {hasDiscount && <tr><td colSpan={5} className="tv-td-right">Discount</td><td className="tv-td-right">-{formatCurrency(invoice.discountTotal || 0)}</td></tr>}
              {(invoice.roundOff || 0) !== 0 && <tr><td colSpan={5} className="tv-td-right">Round Off</td><td className="tv-td-right">{formatCurrency(invoice.roundOff || 0)}</td></tr>}
              <tr><td colSpan={5} className="tv-td-right tv-total">Total</td><td className="tv-td-right tv-total">{formatCurrency(invoice.total || 0)}</td></tr>
            </tfoot>
          </table>
        </section>
        ...
        <section className="tv-bottom">
          ...
          <div className="tv-right">
            ...
            <div className="amount-words-label">Amount chargeable (in words)</div>
            <div className="amount-words">{amountInWords}</div>
            <div className="signature authorization-block">
              <div className="authorization-for">For {seller.name}</div>
