export async function prepareSignatureImage(file: File) {
  if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) throw new Error('Use a PNG, JPEG, or WebP image.');
  if (file.size > 2 * 1024 * 1024) throw new Error('Signature image must be smaller than 2 MB.');
  ...
  if (right <= left || bottom <= top) throw new Error('The signature image appears empty.');
  const width = right - left + 1, height = bottom - top + 1, scale = Math.min(1, 520 / width, 220 / height);
  ...
  if (dataUrl.length > 220_000) throw new Error('Compressed signature is still too large.');
  return dataUrl;
}
