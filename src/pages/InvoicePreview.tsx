    ...
    <div className="mx-auto max-w-6xl space-y-6 pb-12">
      <div className="flex flex-col gap-4 border-b border-stone-200 pb-4 print:hidden sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative flex w-full flex-wrap gap-2 sm:w-auto">
          ...
          {!isEstimate && isAdmin && !['paid', 'cancelled'].includes(invoice.paymentStatus) ? <button type="button" onClick={() => setIsPaymentOpen(true)} className="inline-flex min-h-[48px] items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 font-semibold text-white"><ReceiptIndianRupee size={18} />{t('recordPayment')}</button> : null}
      ...
      {!isEstimate && isAdmin ? (
        <section className="rounded-2xl border border-stone-200 bg-white p-5 print:hidden">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            {!['paid', 'cancelled'].includes(invoice.paymentStatus) ? <button type="button" onClick={() => setIsPaymentOpen(true)} className="rounded-xl bg-blue-600 px-4 py-3 font-bold text-white">{t('recordPayment')}</button> : null}
          </div>
